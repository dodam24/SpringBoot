...
@ComponentScan(basePackages = {"com.example.demo.controller", "board.controller"}) //컴포넌트를 스캔할 하나 이상의 패키지를 지정
//메인 패키지는 자동으로 등록되지만, 다른 패키지를 등록할 때는 @ComponentScan을 꼭 추가해야 한다.
...
만약 루트 패키지인 "com.example"이 아닌 다른 패키지에 클래스를 작성하면 스프링 컨테이너는 해당 클래스를 빈으로 등록하지 않는다. 
